    ...
    <>
      ...
      >
        <button
          ...
          title={canClaim ? "VIP сундук — забрать" : "VIP сундук уже открыт сегодня"}
        >
          <img
      ...
      {open && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60" onClick={() => setOpen(false)}>
          <div className="bg-purple-900 border-2 border-yellow-400 p-4 w-[300px]" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-game text-[12px] text-yellow-300 mb-1 text-center">💎 VIP-сундук</h2>
            <div className="font-game text-[7px] text-purple-200 text-center mb-3">
              ×2-×3 от обычного. Редкий шанс на пыльцу 🌼
            </div>
            {canClaim ? (
              <>
                <div className="bg-purple-950 border border-yellow-500/30 p-3 mb-3 text-center">
                  <div className="font-game text-[7px] text-purple-300 mb-1">Сегодня выпало:</div>
              ...
              <div className="font-game text-[9px] text-purple-200 text-center py-3">
                Уже забрано сегодня.<br/>Возвращайся завтра.
              </div>
            )}
          </div>
        </div>
